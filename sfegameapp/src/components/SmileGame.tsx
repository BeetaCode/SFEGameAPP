...
import { getSmileGame, sendSmileGameData } from '../services/smileGameService';
...
function SmileGame() {
  ...
  const fetchQuestion = async () => {
    try {
      const response = await getSmileGame();
  ...
  const checkAnswer = () => {
    if (userAnswer === answer) {
      if (fullName != 'Guest') {
        ...
        sendSmileGameData(smileGameData);
      }
      setIsCorrect(true);
      setGameOver(true);
    } else {
  ...
  return (
    <div className="h-screen bg-blue-300">
      ...
      <div className="flex flex-col items-center justify-center text-center">
        <h2 className="text-blue-700 text-3xl p-4">Question:</h2>
        {question && (
          <div>
            <img
